package com.sharad.utils.toolbar;
...
public class NavigationView extends RelativeLayout {
    ...
    private final int NAVIGATION_HEIGHT = (int) getResources().getDimension(R.dimen.navigation_height);
    private final int NAVIGATION_LINE_WIDTH = (int) getResources().getDimension(R.dimen.navigation_line_width);
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            withText = array.getBoolean(R.styleable.NavigationView_bnv_with_text, true);
            coloredBackground = array.getBoolean(R.styleable.NavigationView_bnv_colored_background, true);
            disableShadow = array.getBoolean(R.styleable.NavigationView_bnv_shadow, false);
            isTablet = array.getBoolean(R.styleable.NavigationView_bnv_tablet, false);
            viewPagerSlide = array.getBoolean(R.styleable.NavigationView_bnv_viewpager_slide, true);
            itemActiveColorWithoutColoredBackground = array.getColor(R.styleable.NavigationView_bnv_active_color, -1);
            textActiveSize = array.getDimensionPixelSize(R.styleable.NavigationView_bnv_active_text_size, res.getDimensionPixelSize(R.dimen.navigation_text_size_active));
            textInactiveSize = array.getDimensionPixelSize(R.styleable.NavigationView_bnv_inactive_text_size, res.getDimensionPixelSize(R.dimen.navigation_text_size_inactive));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (coloredBackground) {
            ...
            shadowHeight = (int) getResources().getDimension(R.dimen.navigation_shadow_height);
        } else {
            ...
            shadowHeight = (int) getResources().getDimension(R.dimen.navigation_shadow_height_without_colored_background);
        }
        if (isTablet) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                setElevation(getResources().getDimension(R.dimen.navigation_elevation));
            }
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (isTablet) {
            ...
            itemWidth = getWidth() / navigationItems.size();
        ...
        for (int i = 0; i < navigationItems.size(); i++) {
            final int index = i;
            if (!coloredBackground)
                navigationItems.get(i).setColor(white);
            int textActivePaddingTop = (int) context.getResources().getDimension(R.dimen.navigation_padding_top_active);
            int viewInactivePaddingTop = (int) context.getResources().getDimension(R.dimen.navigation_padding_top_inactive);
            int viewInactivePaddingTopWithoutText = (int) context.getResources().getDimension(R.dimen.navigation_padding_top_inactive_without_text);
            final View view = inflater.inflate(R.layout.navigation_item, this, false);
            ...
            viewList.add(view);
            if (navigationItems.get(i).getImageResourceActive() != 0) {
                if (i == currentItem)
                    icon.setImageResource(navigationItems.get(i).getImageResourceActive());
                else
                    navigationItems.get(i).getImageResource();
            } else {
                icon.setImageResource(navigationItems.get(i).getImageResource());
                icon.setColorFilter(i == currentItem ? itemActiveColorWithoutColoredBackground : itemInactiveColor);
            }
            if (i == currentItem) {
                container.setBackgroundColor(navigationItems.get(index).getColor());
            ...
            title.setText(navigationItems.get(i).getTitle());
    ...
    private void onNavigationItemClick(final int itemIndex) {

        if (currentItem == itemIndex) {
            return;
        }

        int viewActivePaddingTop = (int) context.getResources().getDimension(R.dimen.navigation_padding_top_active);
        int viewInactivePaddingTop = (int) context.getResources().getDimension(R.dimen.navigation_padding_top_inactive);
        int viewInactivePaddingTopWithoutText = (int) context.getResources().getDimension(R.dimen.navigation_padding_top_inactive_without_text);
        int centerX;
        int centerY;
        for (int i = 0; i < viewList.size(); i++) {
            if (i == itemIndex) {
                View view = viewList.get(itemIndex).findViewById(R.id.navigation_container);
                ...
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
                    backgroundColorTemp.setBackgroundColor(navigationItems.get(itemIndex).getColor());
                    ...
                    changeBackgroundColor.addListener(new AnimatorListenerAdapter() {
                        ...
                        public void onAnimationEnd(Animator animation) {
                            ...
                            container.setBackgroundColor(navigationItems.get(itemIndex).getColor());
                        }
                    });
                    changeBackgroundColor.start();
                } else {
                    NavigationUtils.backgroundColorChange
                            (container, navigationItems.get(currentItem).getColor(), navigationItems.get(itemIndex).getColor());
                }
            } else if (i == currentItem) {
                View view = viewList.get(i).findViewById(R.id.navigation_container);
                final TextView title = (TextView) view.findViewById(R.id.navigation_item_title);
                final ImageView icon = (ImageView) view.findViewById(R.id.navigation_item_icon);
                if (navigationItems.get(i).getImageResourceActive() != 0) {
                    icon.setImageResource((navigationItems.get(i).getImageResource()));
        ...
        if (onNavigationItemClickListener != null)
            onNavigationItemClickListener.onNavigationItemClick(itemIndex);
